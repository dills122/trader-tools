...
import { BandWidth } from '../../lib/bollinger-bands/band-width';
const { BollingerBands } = require('technicalindicators');
import { BandWidthDownward, BB } from '../../mocks/BB.mock';
import { buildUniformCandlesFromArray } from '../../test-utils';
...
describe('Bollinger Bands::', function () {
    ...
    describe("isOverBought::", () => {
        it('Should signal over bought', () => {
            const bandWidth = new BandWidth({
                candles: buildUniformCandlesFromArray([6, 6, 7, 6, 7, 7, 9, 8, 8, 9, 10, 11, 11, 12, 10, 10, 11, 12, 10, 11, 11, 11])
            });
        ...
        it('Should NOT signal over bought', () => {
            stubs.BBStub.returns(BB);
            const bandWidth = new BandWidth({
                candles: buildUniformCandlesFromArray([6, 6, 7, 6, 7, 7, 9, 8, 8, 9, 7, 8, 8, 9, 7, 7, 8, 9, 7, 8, 8, 8])
            });
    ...
    describe("isOverSold::", () => {
        it('Should signal over sold', () => {
            const bandWidth = new BandWidth({
                candles: buildUniformCandlesFromArray([3, 3, 4, 3, 4, 5, 3, 3, 4, 2, 2, 1, 1, 3, 3, 3, 4, 2, 2, 1, 2, 4])
            });
        ...
        it('Should NOT signal over sold', () => {
            stubs.BBStub.returns(BB);
            const bandWidth = new BandWidth({
                candles: buildUniformCandlesFromArray([3, 3, 4, 3, 4, 5, 3, 3, 4, 4, 4, 3, 3, 3, 3, 3, 4, 4, 4, 3, 4, 4])
            });

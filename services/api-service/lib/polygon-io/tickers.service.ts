...
export const getTickerSymbolPage = async (
  page: number,
  options?: TickerSymbolRequestOptions
): Promise<RawTickerPageResponse> => {
  try {
    const resp = await polygonIOApiRequest<RawTickerPageResponse>(
      '/reference/tickers',
      {
        page,
        ...options

...
describe('Refiners::', function () {
  ...
  describe('Base::', () => {
    beforeEach(() => {
      ...
      spies.groupBySymbolSpy = sandbox.spy(BaseRefiner.prototype, <any>'groupBySymbol');
      spies.aggregateGroupedBySymbolDataSpy = sandbox.spy(
        BaseRefiner.prototype,
        <any>'aggregateGroupedBySymbolData'
      );
      spies.calculateAggregatedSentimentDataSpy = sandbox.spy(
        BaseRefiner.prototype,
        <any>'calculateAggregatedSentimentData'
      );
      spies.groupSentimentListByTypeSpy = sandbox.spy(BaseRefiner.prototype, <any>'groupSentimentListByType');
      spies.calculateAverageSpy = sandbox.spy(BaseRefiner.prototype, <any>'calculateAverage');
    });
    afterEach(() => {
      sandbox.restore();
    });
    describe('Happy Path::', () => {
      ...
      it('Should execute happy path, two stock symbols', () => {
        sentimentEntities = [
          ...getGenericSentimentResultList(2, 'ABR', 1),
          ...getGenericSentimentResultList(2, 'F', 1)
        ];
        const refiner = new BaseRefiner({
          sentimentData: sentimentEntities
        });
